...
class TDLibClient {
  ...
  // ── Stream de stories ──────────────────────────────────────────────────
  static final _storiesController = ReplaySubject<Map<String, dynamic>>();
  static Stream<Map<String, dynamic>> get storyUpdates => _storiesController.stream;
  ...
  static Future<Messages?> getChatHistory({
    required int chatId,
    int fromMessageId = 0,
    required int offset,
    required int limit,
  ...
  static Future<List<Map<String, dynamic>>?> searchPublicChats({required String query}) async {
    try {
      final jsonMap = {"@type": "searchPublicChats", "query": query};
      var result = await _channel
          .invokeMethod('send', {'json': jsonEncode(jsonMap)})
      ...
      logger.e('searchPublicChats error: $e');
    }
    return null;
  }

  static Future<List<Map<String, dynamic>>> searchChatsByTitle({required String query}) async {
    final results = <Map<String, dynamic>>[];
    final seen = <int>{};

    // 1. Busca local
    try {
      final jsonMap = {"@type": "searchChats", "query": query, "limit": 20};
      ...
      if (result["data"] != null) {
        ...
        for (final c in chats) {
          ...
          if (id != null && !seen.contains(id)) { seen.add(id); results.add(chat); }
        }
      }
    } catch (e) { logger.e('searchChats error: $e'); }

    // 2. Busca no servidor por palavra (grupos, canais, etc)
    try {
      final jsonMap = {"@type": "searchChatsOnServer", "query": query, "limit": 50};
      final result = await _channel
          .invokeMethod('send', {'json': jsonEncode(jsonMap)})
          .timeout(const Duration(seconds: 8));
      if (result["data"] != null) {
    ...
    } catch (e) { logger.e('searchChatsOnServer error: $e'); }

    // 3. Busca global por username
    try {
      final jsonMap = {"@type": "searchPublicChats", "query": query};
      final result = await _channel
          .invokeMethod('send', {'json': jsonEncode(jsonMap)})
    ...
    } catch (e) { logger.e('searchPublicChats error: $e'); }

    return results;
  }

  static Future<Map<String, dynamic>?> sendRaw(Map<String, dynamic> jsonMap) async {
    try {
      var result = await _channel.invokeMethod('send', {'json': jsonEncode(jsonMap)});
      if (result["data"] != null) {
        final data = result["data"] is String
      ...
      logger.e('sendRaw error: $e');
    }
    return null;
  }

  static Future<void> joinChat({required int chatId}) async {
    final jsonMap = {"@type": "joinChat", "chatId": chatId};
    await _channel.invokeMethod('send', {'json': jsonEncode(jsonMap)});
  }
  ...
  static void initTdlibUpdates() {
    _updatesChannel.receiveBroadcastStream().listen((event) {
      ...
      switch (type) {
        ...
        // ── Stories ──────────────────────────────────────────────────────
        case updateStoryConst || updateStoryDeletedConst || updateStoriesConst:
          _storiesController.add(update);
        default:
