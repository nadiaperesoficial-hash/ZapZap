...
class TDLibClient {
  ...
  // Busca por username exato (ex: @grupobr)
  ...
  // Busca por título/palavra — usa searchChats (busca local) + searchPublicChats
  static Future<List<Map<String, dynamic>>> searchChatsByTitle({required String query}) async {
    final results = <Map<String, dynamic>>[];
    final seen = <int>{};

    // 1. Busca local por título
    try {
      final jsonMap = {
        "@type": "searchChats",
        "query": query,
    ...
    // 2. Busca global por username/título
    try {
      final jsonMap = {"@type": "searchPublicChats", "query": query};
      final result = await _channel
          .invokeMethod('send', {'json': jsonEncode(jsonMap)})
  ...
  // Envia qualquer comando TDLib diretamente
