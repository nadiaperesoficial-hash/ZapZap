...
// Fundo do chat estilo LINE — azul acinzentado suave
...
class _ChatPageState extends State<ChatPage> {
  ...
  // ── Barra de gravação (inalterada visualmente — usa tema do Telegram) ──
  ...
  // ── Input estilo Telegram (inalterado) ────────────────────────────────
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle),
                  child:
                      const Center(child: Icon(Icons.send, color: Colors.white)),
                ),
              );
            }
            return Listener(
              ...
              child: Container(
                ...
                child:
                    const Center(child: Icon(Icons.mic_none, color: Colors.white)),
              ),
            );
          },
        ),
  ...
  Widget _buildMessageInput() {
    ...
    return ValueListenableBuilder<bool>(
      ...
      builder: (context, isRecording, child) {
        return isRecording ? _buildRecordingBar() : _buildNormalInput();
      },
    );
  }

  // ── Status do usuário (last seen) ────────────────────────────────────
  Widget _buildSubtitle() {
    if (widget.chat['user'] != null) {
      final statusText =
          MessageFormatter.getUserStatus(widget.chat['user']!);
      final isOnline = widget.chat['user']?['status']?['@type'] ==
          'userStatusOnline';
      return Text(
        statusText,
        style: TextStyle(
          fontSize: 12,
          color: isOnline ? const Color(0xFF4CAF50) : Colors.white70,
          fontWeight:
    ...
    if (widget.chat['supergroup'] != null) {
      ...
      return Text(
        '${NumberFormat('#,###', 'en_US').format(count)} subscribers',
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        // ── AppBar estilo LINE ─────────────────────────────────────────
        appBar: AppBar(
          backgroundColor: const Color(0xFF3D4D6A),
                        ...
                        itemBuilder: (context, index) {
                          if (isLoading && index == messages.length) {
                            return const Padding(
                              ...
                              child: Center(
                                child: Text('Loading older messages',
                                    style:
