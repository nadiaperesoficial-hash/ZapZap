...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _loadBatch() async {
    ...
    final firstVisibleIndex =
        (_messages.value.isNotEmpty && pos.maxScrollExtent > 0)
            ? (pos.pixels / (pos.maxScrollExtent / _messages.value.length))
                .round()
  ...
  // ── Envia vídeo circular via Cloudinary + Supabase ─────────────────────
  Future<void> _sendVideoNote(String filePath) async {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Enviando vídeo circular...'),
          backgroundColor: Color(0xFF00BFA5),
          duration: Duration(seconds: 10)),
    );

    try {
      final mediaUrl = await _uploadVideoToCloudinary(filePath);
      if (mediaUrl == null) {
        if (!mounted) return;
      ...
      // Envia link via mensagem de texto com identificador especial
      final noteId = record['id'] as String;
      await TDLibClient.sendMessage(
        chatId: widget.chat['id'],
        text: '🎥 [video_note:$noteId]',
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Vídeo circular enviado! Some em 1h ou após assistir.'),
            backgroundColor: Color(0xFF00BFA5)),
      );
    } catch (e) {
      if (!mounted) return;
      ...
      builder: (_) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            _attachOption(Icons.image_outlined, 'Foto da galeria',
                const Color(0xFF00BFA5), () async {
            ...
            _attachOption(Icons.camera_alt_outlined, 'Câmera',
                const Color(0xFF5C6BC0), () async {
              Navigator.pop(context);
              final picker = ImagePicker();
              final picked = await picker.pickImage(
            ...
            _attachOption(Icons.videocam_outlined, 'Vídeo da galeria',
              ...
              await TDLibClient.sendRaw({
                ...
                'inputMessageContent': {
                  ...
                  'video': {'@type': 'inputFileLocal', 'path': picked.path},
                  'caption': {'@type': 'formattedText', 'text': ''},
                  'duration': 0,
                  'width': 0,
                  'height': 0,
            ...
            _attachOption(Icons.radio_button_checked, 'Vídeo circular',
                const Color(0xFFAAE063), () async {
              Navigator.pop(context);
              final picker = ImagePicker();
              ...
              if (picked == null) return;
              await _sendVideoNote(picked.path);
            }),
          ]),
        ),
      ),
