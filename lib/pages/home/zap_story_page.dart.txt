    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Story postado com sucesso!'),
...
// ── Card de story estilo KakaoStory ──────────────────────────────────────────
class _StoryCard extends StatelessWidget {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Reações ─────────────────────────────────────
          Row(children: [
            _reactionBtn(Icons.favorite_border_rounded,
                const Color(0xFFE53935), ''),
            const SizedBox(width: 16),
            _reactionBtn(Icons.chat_bubble_outline_rounded,
  ...
  Widget _buildMedia(Map<String, dynamic>? content) {
    ...
    if (type == 'storyContentPhoto') {
      ...
      final path = sizes?.isNotEmpty == true
          ? sizes!.last['photo']?['local']?['path']
          : null;
  ...
  Widget _reactionBtn(IconData icon, Color color, String count) {
    return Row(children: [
