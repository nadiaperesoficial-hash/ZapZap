...
const _cloudApiKey = '233388443455679';
const _cloudApiSecret = 'i3mTBpZKWMWF3I-IEDun7R3bYPM';

// Limites
const int _maxVideoSeconds = 60;
const int _maxPhotoBytes = 5 * 1024 * 1024; // 5MB
...
class _ZapStoryPageState extends State<ZapStoryPage> {
  ...
  void initState() {
    ...
    // Escuta stories do TDLib (leitura)
  ...
  Future<void> _loadStories() async {
    ...
    try {
      ...
      // Deleta stories expirados do Supabase
      await Supabase.instance.client
          .from('stories')
          .delete()
          .lt('expires_at', DateTime.now().toIso8601String());

      // Busca stories do Supabase
  ...
  Future<String?> _uploadToCloudinary(String filePath, bool isVideo) async {
    try {
      ...
      final toSign = '$paramsToSign$_cloudApiSecret';
      ...
      final request = http.MultipartRequest('POST', uri)
        ..fields['api_key'] = _cloudApiKey
        ..fields['timestamp'] = timestamp.toString()
        ..fields['signature'] = signature
        ..files.add(await http.MultipartFile.fromPath('file', filePath));

      // Limites de transformação no Cloudinary
      if (isVideo) {
        request.fields['transformation'] = 'q_auto,vc_auto,du_60';
      } else {
      ...
      if (size > _maxPhotoBytes) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Foto muito grande! Máximo 5MB.'),
    ...
    try {
      ...
      if (mediaUrl == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Erro ao enviar mídia. Tente novamente.'),
                backgroundColor: Colors.red),
          );
        }
        return;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Story postado!'), backgroundColor: _kGreen),
        );
        _loadStories();
      }
    } catch (e) {
...
class _StoryCardState extends State<_StoryCard> {
  ...
  Widget _buildMedia(String url, String type) {
    ...
    // Vídeo — mostra thumbnail com play
    return ClipRRect(
      borderRadius: BorderRadius.circular(12),
