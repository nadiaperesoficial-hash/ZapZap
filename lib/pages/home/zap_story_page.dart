...
class _ZapStoryPageState extends State<ZapStoryPage> {
  ...
  Future<void> _loadStories() async {
    try {
      ...
      // Busca stories próprios primeiro
      try {
        final myId = _me?['id'];
        if (myId != null) {
      ...
      // Busca stories de contatos
      final contacts = await TDLibClient.getContacts();
      for (final contact in contacts ?? []) {
        try {
          // No TDLib, para usuários o chatId = userId
  ...
  Future<void> _postStory() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Aguarda TDLib processar o story
