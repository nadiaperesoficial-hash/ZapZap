...
class _ZapStoryPageState extends State<ZapStoryPage> {
  // userId -> lista de stories
  final Map<int, List<Map<String, dynamic>>> _storiesMap = {};
  ...
  void initState() {
    ...
    // Escuta updates de stories em tempo real
    _storySub = TDLibClient.storyUpdates.listen((update) {
      final type = update['@type'] as String?;
      if (type == 'UpdateStory') {
        final story = update['story'] as Map<String, dynamic>?;
        if (story == null) return;
        final senderId = story['senderChatId'] as int? ?? story['chatId'] as int? ?? 0;
        if (senderId == 0) return;
        if (mounted) {
          setState(() {
            _storiesMap.putIfAbsent(senderId, () => []);
            // Atualiza ou adiciona
            final idx = _storiesMap[senderId]!
                .indexWhere((s) => s['id'] == story['id']);
            if (idx >= 0) {
              _storiesMap[senderId]![idx] = story;
            } else {
              _storiesMap[senderId]!.insert(0, story);
            }
          });
          // Baixa mídia do story
          _downloadStoryMedia(story);
        }
      } else if (type == 'UpdateStoryDeleted') {
        final chatId = update['chatId'] as int?;
        final storyId = update['storyId'] as int?;
        if (chatId != null && storyId != null && mounted) {
          setState(() {
            _storiesMap[chatId]?.removeWhere((s) => s['id'] == storyId);
            if (_storiesMap[chatId]?.isEmpty == true) {
              _storiesMap.remove(chatId);
            }
          });
        }
      }
    ...
    try {
      ...
      if (type == 'storyContentPhoto') {
        ...
        if (sizes != null && sizes.isNotEmpty) {
          ...
          if (fileId != null) await TDLibClient.downloadFile(fileId: fileId);
        }
  ...
  Future<void> _loadStories() async {
    ...
    try {
      ...
      final myId = _me?['id'] as int?;

      // Carrega stories próprios
      if (myId != null) {
        _usersMap[myId] = _me!;
        try {
          ...
          if (result != null && result['@type'] != 'error') {
            final list = result['stories'] as List? ?? [];
            for (final s in list) {
              final story = s as Map<String, dynamic>;
              _storiesMap.putIfAbsent(myId, () => []).add(story);
              _downloadStoryMedia(story);
            }
          }
        } catch (_) {}
      ...
      for (final contact in contacts ?? []) {
        ...
        try {
          ...
          if (result != null && result['@type'] != 'error') {
            ...
            for (final s in list) {
              final story = s as Map<String, dynamic>;
              _storiesMap.putIfAbsent(userId, () => []).add(story);
              _downloadStoryMedia(story);
            }
          }
        } catch (_) {}
  ...
  // Todos os stories em lista plana
  List<Map<String, dynamic>> get _allStories {
    final list = <Map<String, dynamic>>[];
    for (final entry in _storiesMap.entries) {
      final user = _usersMap[entry.key];
      for (final story in entry.value) {
        list.add({'story': story, 'user': user});
      }
    }
    return list;
  }
  ...
  Future<void> _postStory() async {
    ...
    try {
      final result = await TDLibClient.sendRaw({
        ...
        'privacySettings': {'@type': 'storyPrivacySettingsEveryone'},
        'activePeriod': 86400,
      ...
      if (result == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Erro: TDLib não suporta stories nesta versão.'),
      ...
      if (result['@type'] == 'error') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Erro $code: $message'),
              backgroundColor: Colors.red,
      ...
      // Story criado — o updateStory vai chegar pelo stream automaticamente
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Story postado!'),
                ...
                : stories.isEmpty
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (_, i) => _StoryCard(data: stories[i]),
                        ),
                      ),
          ),
        ]),
    ...
    if (type == 'storyContentPhoto') {
      ...
      return Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFF0F0F0),
            borderRadius: BorderRadius.circular(12)),
        child: const Center(child: CircularProgressIndicator(color: _kLineGreen)),
      );
    }
