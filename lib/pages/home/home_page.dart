    ...
    TDLibClient.chatUpdates.listen((update) async {
      ...
      switch (type) {
        ...
        case updateChatFoldersConst:
          ...
          if (_tabController == null || _tabController!.length != newFolders.length) {
            ...
            _tabController = TabController(length: newFolders.length, vsync: this)
              ..addListener(() { selectedFolderIndex = _tabController!.index; });
          }
        ...
        case updateChatLastMessageConst:
          ...
          final existingChat = chats.value[chatId];
          if (existingChat != null) {
            final mergedPositions = List<Map<String, dynamic>>.from(newPositions?.map((e) => e) ?? []);
            ...
            final updatedChats = chats.value;
            updatedChats[chatId] = {...existingChat, 'lastMessage': lastMessage, 'positions': mergedPositions};
            chats.value = updatedChats;
          }
        ...
        case updateSupergroupConst:
          ...
          if (type == "ChatMemberStatusLeft" || type == "ChatMemberStatusBanned") isMember = false;
          ...
          if (user != null) {
            ...
            for (final chatId in updatedChats.keys) {
              ...
              if (chat?['user']?['id'] == userId) {
                updatedChats[chatId] = {...?chat, 'user': {...chat?['user'], 'status': update['status']}};
              }
            }
            chats.value = updatedChats;
          }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: !_isSearching && folders.value.isNotEmpty && _tabController != null
            ? PreferredSize(
      ...
      body: _isSearching
          ...
          : folders.value.isEmpty || _tabController == null
              ? ChatListView(
                  chatsNotifier: chats,
                  folderId: null,
